...
- Set `DeadLetterProducerBatchBytes` to split direct dead-letter ProduceBatch calls by approximate uncompressed message bytes. The default `0` keeps the current single-batch behavior. Configure this below the Kafka/topic max message size to leave room for Kafka protocol and request overhead. A single message larger than the limit is still sent alone and can still fail with Kafka `Message Size Too Large`; adjust DLQ topic/broker limits or payload handling for that case.
...
| `deadLetterProducerBatchBytes`                   | Optional byte limit for direct dead-letter producer batches, based on approximate uncompressed message size. Zero disables chunking and preserves current behavior. Set below Kafka/topic max message size to leave protocol overhead room. Works with producer compression, but does not replace proper Kafka topic/broker message-size limits.                                                          | 0                           |
...
| `retryConfiguration.producerBatchBytes`          | Optional byte limit for retry producer batches, based on approximate uncompressed message size. Zero disables retry producer chunking and preserves current behavior. Requires kafka-cronsumer v1.6.11+.                                                                                                                                                                                                  | 0                           |
